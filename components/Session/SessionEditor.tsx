...
import { ActiveSession, CompleteSession, CreateSession, PlannedSession, UpdateSession } from './SessionControl';
import {
  Card,
  Stack,
  TextInput,
    ...
    <Card radius="md" shadow="xs" padding="1rem" w="66%" h="25em">
      ...
      <form onSubmit={create}>
          ...
          <Grid grow align="center">
            ...
            <Grid.Col span={6}>
            <DateTimePicker withSeconds label="Session Time" placeholder="Pick date and time" {...form.getInputProps('date') } />
            </Grid.Col>
            <Grid.Col span={6}>
              ...
              />
              <Group>
                {props.session.state != SessionState.PLANNED && <Button color="blue" onClick={() => {PlannedSession(props.session.id);router.refresh()}}>Set to Planned</Button> }
                {props.session.state != SessionState.ACTIVE && <Button color="orange" onClick={() => {ActiveSession(props.session.id);router.refresh()}}>Set to Active</Button> }
                {props.session.state != SessionState.COMPLETE && <Button color="green" onClick={() => {CompleteSession(props.session.id);router.refresh()}}>Complete Session</Button> }
              </Group></Stack>
            </Grid.Col>
            <Grid.Col span={4}>
              <NumberInput
