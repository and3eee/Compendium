...
export async function RegisterGMFromEmail(campaignID: number, email: string) {
  ...
  const gms: GMProfile[] = (
    await prisma.campaign.findFirstOrThrow({ where: { id: campaignID }, include: { gms: true } })
  )?.gms;
  const gmIds: { id: string }[] = gms.map((gm: GMProfile) => {
    return { id: gm.id };
  });
  gmIds.push({ id: gmProfile!.id });
  return await prisma.campaign.update({
    where: { id: campaignID },
    data: { gms: { connect: gmIds } },
  });
}
...
export async function RegisterUserFromEmail(campaignID: number, email: string) {
  ...
  const memberIds: { id: string }[] = members.map((gm: PlayerProfile) => {
    return { id: gm.id };
  });
  memberIds.push({ id: PlayerProfile!.id });
  return await prisma.campaign.update({
    where: { id: campaignID },
    data: { members: { connect: memberIds } },
  });
}
